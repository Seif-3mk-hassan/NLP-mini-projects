...
tokenized_data = [tokenize_and_preprocess(text,is_tfidf=True) for text in X]

'''word2vec_model = Word2Vec(
    tokenized_data, vector_size=100, window=5, min_count=1, workers=4
)'''

x_train, x_test, y_train, y_test = vectorize_text(
    tokenized_data, y, is_TFIDF=True
)
...
x_train_tensor = torch.tensor(x_train.toarray(), dtype=torch.float).unsqueeze(1)  # (n_emails, seq_len, 100)
x_test_tensor  = torch.tensor(x_test.toarray(),  dtype=torch.float).unsqueeze(1)

input_size  = x_train.shape[1]  # 100 (Word2Vec vector size)
proj_size = 256
...
class RNN(nn.Module):
    def __init__(self,input_size,proj_size, hidden_size,num_layers, num_classes):
        super(RNN, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.input_proj = nn.Sequential(
            nn.Linear(input_size, proj_size),  # 20000 → 256
            nn.ReLU(),
            nn.Dropout(0.3)
        )
        self.lstm = nn.LSTM(
...
model     = RNN(input_size,proj_size, hidden_size, layers, output_size).to(device)
